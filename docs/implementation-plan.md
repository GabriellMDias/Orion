...
| [2](#phase-2) | CI and dependency security | blocked | 1 | [PR #1](https://github.com/GabriellMDias/Orion/pull/1) merged; [H-01](human-actions.md#h-01) and [H-02](human-actions.md#h-02) are verified complete. The active `Protect main` ruleset requires PRs and the `Orion required gate`. Conditional security settings remain under [H-03](human-actions.md#h-03). |
...
| P2.6 | Configure and verify branch protection and supported GitHub security capabilities; record entitlement limitations. | blocked | H-02 is complete: active repository ruleset `Protect main` (ID `23941496`) targets the default branch, requires pull requests and `Orion required gate`, blocks deletion and force pushes, and has no bypass actors. Dependency graph is available and vulnerability alerts are enabled. Remaining conditional feature availability is tracked by [H-03](human-actions.md#h-03). |
