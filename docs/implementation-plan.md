...
Orion now contains documentation, a pnpm workspace, and local validation tooling. `pnpm validate` runs the checks listed in [validation](validation.md). Applications, shared runtime packages, application tests, generators, and CI workflows do not yet exist. Creating this plan and its companion checklist did not itself start an implementation phase; Phase 1 progress is recorded below.
...
| [2](#phase-2) | CI and dependency security | in progress | 1 | Governing ADR and repository access reviewed; workflow, dependency automation, and settings verification underway. |
...
| [8](#phase-8) | Safe evolution and reproducible artifacts | pending | 5-7; actual baselines where applicable | Not started. |
...
| Task | Main work | Status | Evidence / dependency |
| --- | --- | --- | --- |
| P2.1 | Add GitHub Actions validation for pull requests, the primary branch, and manual execution. | in progress | `.github/workflows/ci.yml` declares all triggers; local workflow lint passed. A real remote run is still required. |
| P2.2 | Consume repository-pinned toolchain versions, frozen installation, and repository commands rather than CI-only correctness logic. | in progress | `pnpm/setup` reads `packageManager` and `.node-version`, installs from the frozen lockfile, and CI invokes `pnpm validate`; remote equivalence is unverified. |
| P2.3 | Expose a stable aggregate required check that fails when required work fails or does not complete. | in progress | `Orion required gate` evaluates all required job conclusions with `always()`; remote positive and negative behavior remain to verify. |
...
| P2.5 | Configure Renovate with its dashboard, weekly routine updates, coherent groups, visible major upgrades, and automerge disabled initially; do not delay security remediation to the routine window. | in progress | `renovate.json` passes official strict validation; external app activation and Dashboard behavior remain [H-01](human-actions.md#h-01). |
| P2.6 | Configure and verify branch protection and supported GitHub security capabilities; record entitlement limitations. | blocked | Dependency graph is available and vulnerability alerts were enabled/verified. GitHub returned HTTP 403 for private-repository protection with “Upgrade to GitHub Pro or make this repository public”; see [H-02](human-actions.md#h-02). Other conditional security features remain under [H-03](human-actions.md#h-03). |
...
| Task | Main work | Status | Evidence / dependency |
| --- | --- | --- | --- |
| P5.1 | Implement selected domain rules and application operations in cohesive feature boundaries. | pending | P3.1-P3.3. |
| P5.2 | Introduce PostgreSQL/Prisma 7 for actual durable data, reviewed migrations, constraints, explicit transactions, and persistence adapters. | pending | [H-06](human-actions.md#h-06) only if host setup needs human action. |
| P5.3 | Separate runtime and migration credentials. | pending | Local synthetic credentials; [H-07](human-actions.md#h-07) only for real external prerequisites. |
| P5.4 | Implement required identity verification and authorization before exposing protected operations; test with synthetic identities. | pending | [H-05](human-actions.md#h-05), [H-07](human-actions.md#h-07) if applicable. |
| P5.5 | Define TypeBox requests/responses, unique stable operation IDs, expected errors, and deliberate transport mappings. | pending | Not started. |
| P5.6 | Generate OpenAPI 3.1.x from executable contracts and route metadata. | pending | Not started. |
| P5.7 | Document every application-owned table/column, including ownership, relevant classification, units, null semantics, and lifecycle. | pending | Not started. |
| P5.8 | Generate physical database reference from migrated PostgreSQL, accounting for custom SQL. | pending | Not started. |
| P5.9 | Add Vitest unit and integration tests using Testcontainers, real PostgreSQL, committed migrations, and Fastify injection. | pending | Not started. |
...
| Task | Main work | Status | Evidence / dependency |
| --- | --- | --- | --- |
| P8.1 | Establish durable migration release-status detection before the first persistent release. | pending | [H-08](human-actions.md#h-08) for owner/environment facts unavailable from evidence. |
| P8.2 | Protect released migration history while preserving safe refinement of unreleased history. | pending | Not started. |
| P8.3 | Test fresh installation and upgrades from actual supported released baselines once they exist. | pending | Conditional historical tests; never invent a released baseline. |
| P8.4 | Introduce released API baselines and compatibility checks when independently evolving consumers require them. | pending | [H-08](human-actions.md#h-08) if support obligations require owner input. |
| P8.5 | Review structural/semantic compatibility of errors, authorization, ordering, pagination, defaults, and side effects. | pending | Not started. |
| P8.6 | Verify generators/builds from clean canonical inputs; document safe application rollback versus database forward recovery. | pending | Not started. |
| P8.7 | Define removal conditions for real temporary compatibility paths. | pending | Conditional on such paths existing. |
