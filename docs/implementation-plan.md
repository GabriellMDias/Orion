...
| [5](#phase-5) | Secure persistence-backed API feature | completed | 3-4 and CI | The Approval Request server, PostgreSQL/Prisma migration, JWT boundary, generated references, 35 tests, and emitted feature smoke pass locally and in [PR #6 CI run #4](https://github.com/GabriellMDias/Orion/actions/runs/36055772654), including Dependency Review and the required gate. H-06 needs no host action; H-07 remains conditional. |
...
| P5.9 | Add Vitest unit and integration tests using Testcontainers, real PostgreSQL, committed migrations, and Fastify injection. | completed | 35 tests and emitted-process feature smoke pass locally and in [PR #6 CI run #4](https://github.com/GabriellMDias/Orion/actions/runs/36055772654). Coverage includes signed-token verification, auth denials, idempotent/concurrent creation, CAS races, terminal invariants, cursor scope, schema permissions, and readiness. |
