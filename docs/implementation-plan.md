...
| [5](#phase-5) | Secure persistence-backed API feature | in progress | 3-4 and CI | The server and prior gate passed, but PR #6 CodeQL reported missing rate limiting on database-backed Approval Request routes. A scoped HTTP limiter, tests, and generated contract update are in progress; CodeQL resolution remains to be verified. H-06 needs no host action; H-07 remains conditional. |
...
| P5.5 | Define TypeBox requests/responses, unique stable operation IDs, expected errors, and deliberate transport mappings. | in progress | Eight feature contracts and the central registry exist. A `429 RATE_LIMITED` mapping, retry guidance, and CodeQL verification are being added to the HTTP boundary. |
...
| P5.9 | Add Vitest unit and integration tests using Testcontainers, real PostgreSQL, committed migrations, and Fastify injection. | in progress | Prior 35 tests and emitted-process feature smoke passed locally and in [PR #6 CI run #4](https://github.com/GabriellMDias/Orion/actions/runs/36055772654). Deterministic limit, pre-auth rejection, and health-isolation tests are being added for the CodeQL follow-up. |
...
**Prior implementation evidence (2026-09-24):** `pnpm install --frozen-lockfile` and `pnpm validate` passed locally on Node.js 24.13.0 / pnpm 11.25.0 after the dependency overrides and PostgreSQL fixture correction. The gate generated the ignored Prisma client, checked format/lint/types/import boundaries/docs, recreated the migrated PostgreSQL reference, ran 35 Vitest tests against Testcontainers, built emitted ESM, and passed both foundation and signed-token/migrated-database real-process smokes. [PR #6 CI run #5](https://github.com/GabriellMDias/Orion/actions/runs/36056116063) passed Validate, Dependency Review with `DEPENDENCY_REVIEW_ENABLED=true`, and `Orion required gate` on Ubuntu. The migration was applied on fresh PostgreSQL; the runtime test role has no DDL, delete, or immutable-column update privilege. The API enforces human identity, owner/reviewer scope, self-review denial, state rules, version CAS, and create-key uniqueness. [H-06](human-actions.md#h-06) required no host intervention; [H-07](human-actions.md#h-07) remains conditional because no provider was selected. No tenant applies to this feature. The web application and generated frontend client remain Phase 6 work. CodeQL subsequently identified missing feature-level rate limiting, so Phase 5 completion is pending the current follow-up's local and remote verification.
