...
| [2](#phase-2) | CI and dependency security | in progress | 1 | [H-01](human-actions.md#h-01) and [H-02](human-actions.md#h-02) are complete. Public-repository security configuration and `DEPENDENCY_REVIEW_ENABLED=true` are verified; waiting for an enabled Dependency Review run to pass before completing [H-03](human-actions.md#h-03) and Phase 2. |
...
| P2.6 | Configure and verify branch protection and supported GitHub security capabilities; record entitlement limitations. | in progress | H-02 is complete. GitHub reports the repository public, CodeQL default setup `configured`, secret scanning and push protection enabled, and Actions variable `DEPENDENCY_REVIEW_ENABLED=true`; a new PR run must confirm Dependency Review executes successfully before [H-03](human-actions.md#h-03) can complete. |
