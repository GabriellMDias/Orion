...
| [5](#phase-5) | Secure persistence-backed API feature | in progress | 3-4 and CI | Local feature validation passes, but [PR #6 CI run #1](https://github.com/GabriellMDias/Orion/actions/runs/36053541027) found vulnerable Prisma dependencies. A scoped `deepmerge-ts` fix removed the first finding; [run #2](https://github.com/GabriellMDias/Orion/actions/runs/36054529635) exposed `mysql2@3.15.3`. Both patched-version overrides require revalidation. H-06 needs no host action; H-07 remains conditional. |
...
| P5.2 | Introduce PostgreSQL/Prisma 7 for actual durable data, reviewed migrations, constraints, explicit transactions, and persistence adapters. | in progress | Migration, schema, adapter, and migrated-container tests pass. [PR #6 CI runs #1-2](https://github.com/GabriellMDias/Orion/pull/6/checks) found high-severity advisories in Prisma's transitive `deepmerge-ts@7.1.5` and `mysql2@3.15.3`; verify the scoped patched-version overrides through the full gate. |
...
| P5.5 | Define TypeBox requests/responses, unique stable operation IDs, expected errors, and deliberate transport mappings. | completed | Eight feature contracts share route metadata, stable IDs, error declarations, and the central public registry; injection tests exercise 400/401/403/404/409 outcomes. |
...
| P5.9 | Add Vitest unit and integration tests using Testcontainers, real PostgreSQL, committed migrations, and Fastify injection. | completed | 35 tests passed, including signed-token verification, auth denials, idempotent/concurrent creation, CAS races, terminal invariants, cursor scope, schema permissions, and readiness; emitted-process feature smoke passed. |
...
**Local implementation evidence (2026-09-24):** `pnpm install --frozen-lockfile` and `pnpm validate` passed locally on Node.js 24.13.0 / pnpm 11.25.0 before the dependency-review finding. The gate generated the ignored Prisma client, checked format/lint/types/import boundaries/docs, recreated the migrated PostgreSQL reference, ran 35 Vitest tests against Testcontainers, built emitted ESM, and passed both foundation and signed-token/migrated-database real-process smokes. The migration was applied on fresh PostgreSQL; the runtime test role has no DDL, delete, or immutable-column update privilege. The API enforces human identity, owner/reviewer scope, self-review denial, state rules, version CAS, and create-key uniqueness. [H-06](human-actions.md#h-06) required no host intervention; [H-07](human-actions.md#h-07) remains conditional because no provider was selected. No tenant applies to this feature. The web application and generated frontend client remain Phase 6 work. The remote Validate job passed in [PR #6 CI run #1](https://github.com/GabriellMDias/Orion/actions/runs/36053541027), but the required gate failed due to Dependency Review. Phase 5 remains open pending remediation and revalidation.
