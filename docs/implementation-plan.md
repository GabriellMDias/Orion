...
| [5](#phase-5) | Secure persistence-backed API feature | in progress | 3-4 and CI | Local feature validation and [PR #6 CI run #3](https://github.com/GabriellMDias/Orion/actions/runs/36055107134) Dependency Review pass with scoped patched Prisma dependencies. Linux validation exposed a PostgreSQL fixture readiness race; revalidation is pending. H-06 needs no host action; H-07 remains conditional. |
...
| P5.2 | Introduce PostgreSQL/Prisma 7 for actual durable data, reviewed migrations, constraints, explicit transactions, and persistence adapters. | completed | Reviewed migration, schema, adapter, PostgreSQL constraints, atomic create/CAS writes, and local migrated-container tests pass. Scoped patched Prisma dependencies pass [PR #6 CI run #3](https://github.com/GabriellMDias/Orion/actions/runs/36055107134) Dependency Review. |
...
| P5.9 | Add Vitest unit and integration tests using Testcontainers, real PostgreSQL, committed migrations, and Fastify injection. | in progress | 35 tests and emitted-process feature smoke pass locally. [PR #6 CI run #3](https://github.com/GabriellMDias/Orion/actions/runs/36055107134) exposed an initialization/final-server readiness race in the Linux PostgreSQL fixture; verify the corrected wait on both hosts. |
