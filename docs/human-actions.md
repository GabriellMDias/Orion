...
**Local workflow evidence (2026-09-25):** The [manual setup](setup.md#manually-exercise-approval-requests) ran the real JWT-verifying API with a disposable PostgreSQL and loopback synthetic issuer. HTTP checks observed anonymous `401`, owner create/submit success, self-review `403`, reviewer approval success, and stale-write `409`; Ctrl+C removed the temporary token files and container. Owner and reviewer tokens are written only to ignored temporary local files for manual use. The full `pnpm validate` gate passed. No external provider, account, credential, or H-07 human action is required for this development workflow; H-07 stays conditional.
