...
All Approval Request routes share a process-local, source-IP rate limit of 120 requests per minute. The `onRequest` check runs before token verification and database access; health routes are outside its scope. Exhausted clients receive `429 RATE_LIMITED` in the public error envelope with a bounded `Retry-After` value in seconds. The threshold protects the feature's database-backed reads and writes without defining a product quota. The limiter uses the direct peer address because the API does not yet configure trusted proxies. A deployment with multiple replicas or a trusted reverse proxy must define a shared/edge limit and trusted client-IP handling before relying on a fleet-wide or per-user guarantee; this phase introduces neither infrastructure nor a provider-specific assumption.
...
The [release and evolution workflow](../../docs/database/release-evolution.md) explains how a real durable migration boundary is recorded and protected. No durable release is currently recorded; an empty registry is not permission to rewrite migration history without verifying its status.
